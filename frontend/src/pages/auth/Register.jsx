...
const Register = () => {
  ...
  const [form, setForm] = useState({
    name: '', email: '', password: '', businessName: '', businessType: '', phone: '',
  });
  ...
  return (
    <AuthLayout title="Start your free trial" subtitle="14 days free · No credit card required">
      <form onSubmit={handleSubmit} className="space-y-4">
        <Input label="Full Name" placeholder="Priya Sharma" icon={User}
        ...
        <Input label="Business Name" placeholder="Dream Salon" icon={Store}
          value={form.businessName} onChange={(e) => setForm({ ...form, businessName: e.target.value })} required />
        <div className="grid grid-cols-2 gap-3">
          <Select label="Business Type" value={form.businessType}
            onChange={(e) => setForm({ ...form, businessType: e.target.value })} required>
        ...
        <label className="flex items-start gap-2 text-sm text-text-secondary cursor-pointer">
          <input type="checkbox" required className="mt-0.5 rounded border-border-light bg-surface text-primary focus:ring-primary" />
